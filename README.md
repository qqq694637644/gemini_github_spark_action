...
An OAuth-ready, workspace-first GitHub maintenance backend for Gemini Spark custom Connected Apps.
...
- OAuth 2.1 resource-server metadata and `WWW-Authenticate` discovery challenges.
...
- Optional static Bearer mode for local or single-user deployments.
...
- A GitHub App installation or narrowly scoped fine-grained PAT.
- For production: an HTTPS hostname and an OAuth 2.1/OIDC provider capable of issuing access tokens for this resource server.
...
Copy `.env.example` and configure these values through the deployment platform's secret/configuration system:
...
2. Configure the external identity provider for the MCP resource URL and required scopes.
3. Add the HTTPS MCP URL, for example `https://gateway.example.com/mcp`, as a Gemini Spark custom Connected App.
4. Complete the provider's authorization flow or configure client credentials through Spark's supported advanced setup.
...
The remote validator confirms health, exact non-redirecting MCP initialization, protocol negotiation, all 32 tools, and session termination without printing the token.
...
- Prefer a GitHub App with repository-specific installation access.
