...
class WorkspaceCommandRequest(GatewayBaseModel):
    """Legacy REST/OpenAPI-compatible request envelope.

    The legacy action importer requires an operation request body to have an
    object schema at the top level. A Pydantic discriminated union is emitted
    as a top-level ``oneOf`` and is therefore skipped by that importer. This
    flat envelope keeps the compatibility schema object-shaped while
    preserving the strongly typed action-specific models used internally.
    """
